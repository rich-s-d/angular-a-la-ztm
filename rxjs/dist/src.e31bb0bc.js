...
const observable = new _rxjs.Observable(subsriber => {
  subsriber.next('Hello World');
  subsriber.next("Hello World Again"); //subsriber.complete() // stop the observable and memory leak.

  subsriber.next("Hello World Again"); // this will not be executed.

  const id = setInterval(() => {
    subsriber.next("Hello World Again");
  }, 1000); // below is how to save memory leak directly in the observer. But if we want to perpetually subscribe, we can choose to unsubscribe in the observer instead.
